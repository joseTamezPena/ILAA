...
# Effect of UPLTM-Based Decorrelation on Feature Discovery

Here I showcase of to use BSWiMS feature selection/modeling function coupled with Goal Driven Sparse Transformation Matrix (UPLTM) as a pre-processing step to decorrelate highly correlated features. The aim(s) are:
...
#                  scale = "none",
                  mar = c(5,5),
                  col=rev(heat.colors(5)),
                  main = "COVID-19 Correlation after UPLTM",
                  cexRow = 0.35,
                  cexCol = 0.35,
                  key.title=NA,
                  key.xlab="Spearman Correlation",
...
UPLTM <- attr(deIR,"UPLTM")
fscores <- attr(deIR,"fscore")
fscores <- fscores[order(-fscores)]
#fscores[fscores>0]

gplots::heatmap.2(log(abs(1000*UPLTM)+1),
                  trace = "none",
#                  scale = "none",
                  mar = c(5,5),
                  col=rev(heat.colors(5)),
...
```{r results = "asis", warning = FALSE, dpi=300, fig.height= 5.0, fig.width= 6.0}
set.seed(30)
caseSet <- subset(SalivaIR_Avg,class==1)
controlSet <- subset(SalivaIR_Avg,class==0)
trainCasesamples <- sample(nrow(caseSet),nrow(caseSet)/2) 
trainContolsamples <- sample(nrow(controlSet),nrow(controlSet)/2)
trainSet <- rbind(caseSet[trainCasesamples,],controlSet[trainContolsamples,])
testSet <- rbind(caseSet[-trainCasesamples,],controlSet[-trainContolsamples,])

hm <- heatMaps(data=trainSet,
               Outcome="class",
               Scale=TRUE,
               hCluster = "row",
               xlab=expression("Wavenumber (cm)"^"-1"),
...
```{r results = "asis", warning = FALSE, dpi=300, fig.height= 5.0, fig.width= 6.0}
deIR <- IDeA(trainSet,thr=0.80,verbose = TRUE)
preIR <- predictDecorrelate(deIR,testSet)
...
topFeatures <- univariate_BinEnsemble(deIR,Outcome="class")
wavetop <- names(topFeatures)
wavetop <- str_remove_all(wavetop,"La_")
wavetop <- as.numeric(str_remove_all(wavetop,"V_"))
values <- as.numeric(topFeatures)
plot(wavetop,-log10(values),col="red",
     xlim=c(800,1400),
     ylim=c(1.0,2.5),
     ylab="-log(p)",
     xlab=expression("Wavenumber (cm)"^"-1"),
     main="Train Discovery")

topFeatures <- univariate_BinEnsemble(preIR,Outcome="class")
wavetop <- names(topFeatures)
wavetop <- str_remove_all(wavetop,"La_")
wavetop <- as.numeric(str_remove_all(wavetop,"V_"))
values <- as.numeric(topFeatures)
