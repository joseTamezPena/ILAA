...
```{r}
set.seed(2)
trainsamples <- sample(nrow(body_fat),2*nrow(body_fat)/3)
...
obsCoef <- getObservedCoef(trainfat_driven,fatmodel_DE$coef)
pander::pander(obsCoef,"IDeA Modeling")


pred2 <- as.matrix(testingset[,names(obsCoef[-1])]) %*% as.matrix(obsCoef[-1]) + obsCoef[1]
...
trainfat_driven2 <- predictDecorrelate(trainDe,trainingset)
testfat_driven2 <- predictDecorrelate(trainDe,testingset)

pander::pander(t(colnames(trainfat_driven2)))
...
varlistDe <- colnames(trainfat_driven2)
varlistDe <- varlistDe[varlistDe != "BodyFat"]

  par(cex=0.6,cex.main=0.85,cex.axis=0.7)
  cormat <- cor(trainfat_driven2[,varlistDe],method="pearson")
  cormat[is.na(cormat)] <- 0
  gplots::heatmap.2(abs(cormat),
                    trace = "none",
  #                  scale = "row",
...
```{r}
fatmodel_DE2 <- LASSO_1SE(BodyFat~.,trainfat_driven2);
predbfat_DE2 <- predict(fatmodel_DE2,testfat_driven2)
pander::pander(fatmodel_DE2$coef)
devCoef <- fatmodel_DE2$coef
coefnamesLa <- theLaFormulas[names(devCoef)]
coefnames <- coefnamesLa[!unlist(lapply(coefnamesLa,is.null))]
for (cn in names(coefnames))
{
...
obsCoef2 <- getObservedCoef(trainDe,fatmodel_DE2$coef)
pander::pander(obsCoef2,"IDeA Modeling2")
...
pander::pander(fatmodel$coef,"Raw Modeling")
pander::pander(obsCoef,"IDeA 1 Modeling")
pander::pander(obsCoef2,"IDeA 2 Modeling")

lmod <- lm(BodyFat~.,trainingset[,c("BodyFat",names(fatmodel$coef)[-1])])
pander::pander(summary(lmod))

lmod_DE <- lm(BodyFat~.,trainfat_driven[,c("BodyFat",names(fatmodel_DE$coef)[-1])])
pander::pander(summary(lmod_DE))

lmod_DE2 <- lm(BodyFat~.,trainfat_driven2[,c("BodyFat",names(fatmodel_DE2$coef)[-1])])
pander::pander(summary(lmod_DE2))
...
plot(predbfat_DE2,testingset$BodyFat,main="Body Fat Prediction",ylab="Body Fat",xlab="Predictor")
pander::pander(cor.test(testingset$BodyFat,predbfat_DE2))
