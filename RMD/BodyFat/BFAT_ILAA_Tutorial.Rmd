...
that creates a linear transformation from a sample multidimensional data
that effectively removes linear associations between the data variables.
...
3.  Address multicollinerity issues in linear regression models:
...
-   Use ILAA as pre-processing step to model an specific target-outcome
    using linear models
...
from the data. It also computes the Body Mass Index (BMI)
...
-   `…` : Parameters to be passed to the `featureAdjustment()` method
...
The main attributes is "`UPLTM`". That stores the linear transformation
matrix from observed variables to the latent variable. The
"`IDeAEvolution`" attribute can be used to verify if the algorithm
...
parent variable to its linear model.
...
but their model is fully correlated to the observed parent variable. An
...
split the data into training and validations sets. Henceforth, the next
...
data-set from a `ILAA` transformed object. The next lines of code shows
the process of transforming the training set and then use the returned
object to transform the testing set.

```{r results = "asis", warning = FALSE, dpi=300, fig.height= 5.0, fig.width= 7.0}

#body_fat_Decorrelated_train <- ILAA(trainingset,thr=0.2,Outcome="BodyFat",drivingFeatures="BodyFat",verbose=TRUE)
#pander::pander(attr(body_fat_Decorrelated_train,"drivingFeatures"))
body_fat_Decorrelated_train <- ILAA(trainingset,thr=0.2,Outcome="BodyFat",verbose=TRUE)
#pander::pander(attr(body_fat_Decorrelated_train,"drivingFeatures"))
...
next lines of codes shows the procedure.
...
correlated to the real $BodyFat$ $\rho=0.863$
...
latent variables are not collinear hence the statistically significance
of the beta coefficients are not affected by multiculturalism. The next
lines of code shows how to get the beta coefficients using the `lm()` ,
and `summary.lm()` functions.

The inspection of the summary results clearly shows that all the beta
coefficients on the transformed data set are significant while only
three coefficients from the observed data set are statistically
significant.
...
This last experiment showcase the effect of data transformation on
...
target outcome is to identify if the person is Over weight or normal.
(BMI\>=25). The next lines of code compute the new dataframes and remove
...
BMI_Decorrelated_train <- ILAA(trainingsetBMI,thr=0.2,Outcome="Overweight",verbose=TRUE)

BMI_Decorrelated_test <- predictDecorrelate(BMI_Decorrelated_train,testingsetBMI)

```

The last two lines of code transform the observed features using ILLA,
and setting a target variable and setting the convergence not to be
affected by the target outcome.
...
The metrics shows that the model from transformed data set has an
accuracy of 0.905 with 95%CI of 0.825 to 0.968
...
The model created from the observed data has an ROC AUC of 0.921 95%CI
(0.857-0.984), that is not statistically significant to the transformed
model ROC AUC 0.905 95%CI (0.825-0.968)
...
ILAA is an unsupervised computer-based approach for the estimation of
linear transformation matrices that trasforms datasets into a new
latent-based space with a degree of user-controlled correlation. This
report presented a clear example of the use of ILAA as well as all the
functions to estimate, predict and inspect the transformations with
practical applications in supervised learning of regression and logistic
models.
