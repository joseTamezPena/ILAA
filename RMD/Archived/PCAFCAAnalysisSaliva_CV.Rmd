...
for (cvloop in c(1:cvloops))
{
    ...
    tb <- table(trainDataFrame[,outcome])
    aucTHR = power.roc.test(ncontrols=tb[1],ncases=tb[2],sig.level = 0.05/sqrt(ncol(trainDataFrame)),power = 0.20)$auc
    ...
    topSigfeat <- names(univariate_Wilcoxon(trainDataFrame,outcome,thr=rhoThreshold))
    
    topfeat <- (c(1:nrow(univar$orderframe)) <= topFeat) & (rownames(univar$orderframe) %in% topSigfeat)
    ...
    trainBACC <- (univar$orderframe$Sensitivity + univar$orderframe$Specificity)/2.0
    testBACC <- (univarTest$orderframe$Sensitivity + univarTest$orderframe$Specificity)/2.0
    ChangeRawAUCs <- c(ChangeRawAUCs,univarTest$orderframe$ROCAUC[posdisc] - univar$orderframe$ROCAUC[posdisc])
    ChangeRawBACs <- c(ChangeRawBACs,testBACC[posdisc] - trainBACC[posdisc])
