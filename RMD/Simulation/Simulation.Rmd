...
DiscoverySenF <- numeric(NUmberofSimulations)
DiscoverySpeF <- numeric(NUmberofSimulations)
...
DiscoverySenSF <- numeric(NUmberofSimulations)
DiscoverySpeSF <- numeric(NUmberofSimulations)
...
for (i in 1:NUmberofSimulations)
{
  ...
  DiscoverySenT[i] <- evalIDeATRUE$DiscoverySen
  DiscoverySpeT[i] <- evalIDeATRUE$DiscoverySpe
  ...
  DiscoverySenF[i] <- evalIDeAFALSE$DiscoverySen
  DiscoverySpeF[i] <- evalIDeAFALSE$DiscoverySpe
  ...
  DiscoverySenST[i] <- evalIDeASpearmanT$DiscoverySen
  DiscoverySpeST[i] <- evalIDeASpearmanT$DiscoverySpe
  ...
  DiscoverySenSF[i] <- evalIDeASpearmanF$DiscoverySen
  DiscoverySpeSF[i] <- evalIDeASpearmanF$DiscoverySpe


  sizeMat <- nrow(evalIDeATRUE$UsedBetas)^2
  overlapTF[i] <- sum(evalIDeATRUE$UsedBetas == evalIDeAFALSE$UsedBetas)/sizeMat
  overlapSTSF[i] <- sum(evalIDeASpearmanT$UsedBetas == evalIDeASpearmanF$UsedBetas)/sizeMat
  overlapTST[i] <- sum(evalIDeATRUE$UsedBetas == evalIDeASpearmanT$UsedBetas)/sizeMat
  overlapFST[i] <- sum(evalIDeAFALSE$UsedBetas == evalIDeASpearmanT$UsedBetas)/sizeMat
  overlapTSF[i] <- sum(evalIDeATRUE$UsedBetas == evalIDeASpearmanF$UsedBetas)/sizeMat
  overlapFSF[i] <- sum(evalIDeAFALSE$UsedBetas == evalIDeASpearmanF$UsedBetas)/sizeMat
  
}
par(cex=0.8,cex.main=0.85,cex.axis=0.9)

boxplot(list(Pearson_1=DiscoveryAccuracyT,Pearson_2=DiscoveryAccuracyF,Spearman_1=DiscoveryAccuracyST,Spearman_2=DiscoveryAccuracySF),
        main="Accuracy",ylab="Accuracy")
boxplot(list(Pearson_1=DiscoverySenT,Pearson_2=DiscoverySenF,Spearman_1=DiscoverySenST,Spearman_2=DiscoverySenSF),
        main="Sensitivity",ylab="Sensitivity")
boxplot(list(Pearson_1=DiscoverySpeT,Pearson_2=DiscoverySpeF,Spearman_1=DiscoverySpeST,Spearman_2=DiscoverySpeSF),
...
boxplot(list(Pearson_1=trainCorT,Pearson_2=trainCorF,Spearman_1=trainCorST,Spearman_2=trainCorSF),
        main="Train Correlation",
        ylab="Correlation")
boxplot(list(Pearson_1=testCorT,Pearson_2=testCorF,Spearman_1=testCorST,Spearman_2=testCorSF),
...
#                           pOfnoZero= -pOfnoZero)
  ...
  for (thr in thrvals)
  {
    ...
    DiscoverySen[i,cn] <- evalIDeA$DiscoverySen
    DiscoverySpe[i,cn] <- evalIDeA$DiscoverySpe

    rotSparcity[i,cn] <- sum(evalIDeA$UPLTM==0)/(ncol(evalIDeA$UPLTM)^2)
    cn <- cn + 1;
  }
}
...
boxplot(DiscoveryAccuracy,main="Accuracy",ylab="Accuracy")
boxplot(DiscoverySen,main="Sensitivity",ylab="Sensitivity")
boxplot(DiscoverySpe,main="Specificity",ylab="Specificity")
boxplot(0.5*(DiscoverySpe+DiscoverySen),main="Balanced Accuracy",ylab="BACC")
...
boxplot(DiscoverySpe,main="Specificity",ylab="Specificity")
boxplot(0.5*(DiscoverySpe+DiscoverySen),main="Balanced Accuracy",ylab="BACC")
boxplot(testCor,main="Test Correlation",ylab="Correlation")
lines(c(1,length(thrvals)),ylim)
boxplot(falseDiscovery,main="False Discoveries",ylab="Number of False Discoveries")
boxplot(rotSparcity,main="Rotation Matrix Sparcity",ylab="Sparcity")
...
for (rToCRatio in randomToCorrelRatios)
{
  for (i in 1:NUmberofSimulations)
  {
    ...
    evalIDeA <- IDeAEvaluation(traindata=syndata$data[trainSample,],
                                   ...
                                   thr=thr,
                                   type=type)
#    acor <-abs(cor(syndata$data[,str_detect(colnames(syndata$data),"ER")])) 
#    diag(acor) <- 0
#    max(acor)
    ...
    DiscoverySenR[i,cn] <- evalIDeA$DiscoverySen
    DiscoverySpeR[i,cn] <- evalIDeA$DiscoverySpe
